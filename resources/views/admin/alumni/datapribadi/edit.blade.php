        ...
        <section class="mt-10 mx-10 lg:min-h-[55vh]">
            <div class="flex flex-col items-start justify-between md:flex-row">
                <div class="my-5 md:my-0">
                    ...
                    <a href="{{ route('adminDetailAlumni', $data->id_pribadi) }}"
                        class="hidden mt-3 text-blue-500 underline underline-offset-8 md:block">Kembali</a>
                </div>
                <form action="{{ route('updateAlumniPribadi', $data->id_pribadi) }}" method="post" class="w-full">
                    <div class="flex flex-col w-full space-y-5">
                        ...
                        <label class="block text-sm">
                            ...
                            <input type="number" name="nisn"
                                ...
                                placeholder="0123456789" value="{{ old('nisn', $data->nisn) }}" />
                        ...
                        <div class="grid w-full grid-cols-1 gap-5 md:grid-cols-2">
                            ...
                            <label class="block text-sm">
                                ...
                                <select name="agama"
                                    ...
                                    @foreach ($agama as $item)
                                        <option value="{{ $item }}"
                                            {{ old('agama', $data->agama) == $item ? 'selected' : '' }}>
                                            {{ $item }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <label class="block text-sm">
                                ...
                                <input type="tel" name="no_telp"
                                    ...
                                    placeholder="+00 xxx xxx xxx" value="{{ old('no_telp', $data->no_telp) }}" />
                        ...
                        <label class="block text-sm">
                            ...
                            <input name="tmp_lahir"
                                class="block w-full px-5 py-2 mt-1 text-sm border border-gray-600 rounded-md dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                                placeholder="tempat lahir anda" value="{{ old('tmp_lahir', $data->tempat_lahir) }}" />
                            <!-- error -->
                            @error('tmp_lahir')
                                <p class="mt-1 text-rose-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mt-4 text-sm">
                            ...
                            <select name="tamatan" id=""
                                ...
                                @for ($tahun = Carbon\Carbon::now()->year; $tahun >= 2005; --$tahun)
                                    <option value="{{ $tahun }}"
